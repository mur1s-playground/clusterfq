...
int ecdh_generate_keys(uint8_t* pub, uint8_t* priv)
{
    ...
    else
    {
        ...
        /* Multiply base-point with scalar (priv-key) */
...
int ecdh_shared_secret(const uint8_t* priv, const uint8_t* others_pub, uint8_t* output)
{
    /* Do some basic validation of other party's pub key */
    ...
    {
        /* Copy other side's pub key to output */
        unsigned int i;
        ...
        /* Multiply other side's pub key with own priv key */
        gf2point_mul((uint32_t*)output, (uint32_t*)(output + BITVEC_NBYTES), (const uint32_t*)priv);
...
/* ECDSA is broken :( ... */
int ecdsa_sign(const uint8_t* priv, uint8_t* hash, uint8_t* random_k, uint8_t* signature)
{
    /*
    ...
    */
        ...
        && !bitvec_is_zero((uint32_t*)random_k))
    {
        ...
        if (!bitvec_is_zero(r))
        {
            ...
            gf2field_mul(r, r, (uint32_t*)priv); /* r = (r * d) */
...
int ecdsa_verify(const uint8_t* pub, uint8_t* hash, const uint8_t* signature)
{
    /*
    ...
    */
        ...
        && !bitvec_is_zero(r))
    {
        ...
        bitvec_copy(w, (uint32_t*)(pub));
        bitvec_copy(z, (uint32_t*)(pub + ECC_PRV_KEY_SIZE));
